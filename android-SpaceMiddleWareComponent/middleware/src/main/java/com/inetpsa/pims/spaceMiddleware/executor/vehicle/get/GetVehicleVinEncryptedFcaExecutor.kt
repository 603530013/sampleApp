...
internal class GetVehicleVinEncryptedFcaExecutor(
    ...
    override suspend fun execute(input: VehicleImageFcaInput): NetworkResponse<VehicleVinOutput> {
        val request = request(
            type = object : TypeToken<VehicleImageResponse>() {}.type,
            urls = arrayOf("/v1/prelogin/vehicles/details/"),
            queries = mapOf(
                Constants.QUERY_PARAM_KEY_EVIN to input.vin,
        ...
        return communicationManager
            .get<VehicleImageResponse>(request, TokenType.NotConnectedAWSToken(FCAApiKey.SDP))
