...
internal class GetNaftaDealerServicesFcaExecutor(
    ...
    override suspend fun execute(
        ...
        val request = request(
            ...
            urls = arrayOf("/v1/accounts/", uid, "/vehicles/", input.vin, "/servicescheduler/dealerservices"),
            headers = headers(token),
            queries = queries
        )

        return communicationManager.get<NaftaDealerServicesResponse>(request, TokenType.AWSToken(FCAApiKey.SDP))
            .ifSuccess { BookingOnlineCache.write(ServiceType.Dealer, it.services) }
    }
}
