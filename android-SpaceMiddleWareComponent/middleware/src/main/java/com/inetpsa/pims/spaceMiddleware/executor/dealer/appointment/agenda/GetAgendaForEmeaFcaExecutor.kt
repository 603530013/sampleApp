...
internal class GetAgendaForEmeaFcaExecutor(
    middlewareComponent: MiddlewareComponent,
    params: Map<String, Any?>? = null
) : BaseFcaExecutor<AgendaInput, DealerAgendaEMEAResponse>(middlewareComponent, params) {
    ...
    override suspend fun execute(input: AgendaInput): NetworkResponse<DealerAgendaEMEAResponse> {
        var timeFence = input.timeFence
        if (input.timeFence == AgendaInput.TimeFence.SEMESTER) {
        ...
        val request = request(
            type = object : TypeToken<DealerAgendaEMEAResponse>() {}.type,
