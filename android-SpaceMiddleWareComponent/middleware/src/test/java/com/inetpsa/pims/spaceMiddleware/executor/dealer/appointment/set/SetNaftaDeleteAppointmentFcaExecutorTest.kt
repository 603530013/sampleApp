...
internal class SetNaftaDeleteAppointmentFcaExecutorTest : FcaExecutorTestHelper() {

    private lateinit var executor: SetNaftaDeleteAppointmentFcaExecutor
    private val token = "testToken"

    private val input = DeleteNaftaLatamInput(
        vin = "testVin",
    ...
    override fun setup() {
        ...
        executor = spyk(SetNaftaDeleteAppointmentFcaExecutor(middlewareComponent))
    }

    @Test
    fun `when execute then make a get API call`() {
        val departmentId = 3075
        every { executor.params(any()) } returns input
        coEvery {
            communicationManager.delete<DeleteDealerAppointmentResponse>(any(), any())
        } returns Success(response)
        runTest {
            val response = executor.execute(input, token = token, departmentId.toString())
            verify {
                executor.request(
                    type = eq(DeleteDealerAppointmentResponse::class.java),
                    urls = eq(
                        arrayOf(
                            "/v1/accounts/",
                            uid,
