...
internal class GetDealerListNearbyPsaExecutorTest : PsaExecutorTestHelper() {

    private lateinit var executor: GetDealerListNearbyPsaExecutor
    ...
    override fun setup() {
        super.setup()
        executor = spyk(GetDealerListNearbyPsaExecutor(baseCommand))
    }
    ...
    fun `when execute then make a post API call`() {
        ...
        coEvery { communicationManager.get<Dealers>(any(), any()) } returns Success(dealers)
        ...
        runTest {
            ...
            verify {
                executor.request(
                    type = eq(Dealers::class.java),
                    urls = eq(arrayOf("/shop/v1/dealers/near")),
                    queries = eq(queries)
                )
            }

            coVerify {
                communicationManager.get<Dealers>(
                    request = any(),
    ...
    @Test
    fun `when execute params with the right input then return GetDealerListNearbyPsaParams`() {
