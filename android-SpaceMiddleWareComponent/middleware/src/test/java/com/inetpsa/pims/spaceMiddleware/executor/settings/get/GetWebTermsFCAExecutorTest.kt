...
internal class GetWebTermsFCAExecutorTest : FcaExecutorTestHelper() {
    ...
    private lateinit var executor: GetWebTermsFCAExecutor
    ...
    override fun setup() {
        ...
        executor = spyk(GetWebTermsFCAExecutor(baseCommand))
    }
                ...
                anyConstructed<FetchTermConditionFCAExecutor>().execute(
                    eq(
                        TermsConditionsInput(
                            country = "FR",
                            sdp = paramSDP,
                            type = TermsConditionsInput.Type.WebTerms
                        )
                    )
                )
            }
