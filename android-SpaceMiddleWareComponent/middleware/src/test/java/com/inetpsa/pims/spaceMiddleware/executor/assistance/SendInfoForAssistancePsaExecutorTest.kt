...
@Deprecated("try to switch to use this class SetAssistancePSAExecutorTest")
internal class SendInfoForAssistancePsaExecutorTest : PsaExecutorTestHelper() {

    private lateinit var executor: SendInfoForAssistancePsaExecutor

    private val assistance = AssistanceResponse(
        id = "id",
    ...
    override fun setup() {
        super.setup()
        executor = spyk(SendInfoForAssistancePsaExecutor(baseCommand))
    }
    ...
    fun `when execute then make a post API call`() {
        ...
        coEvery { communicationManager.post<AssistanceResponse>(any(), any()) } returns Success(assistance)
        ...
        runTest {
            ...
            Assert.assertEquals(assistance, success.response)
        }
    }
