...
class AssistanceCommandSetTest {

    private val middlewareComponent: MiddlewareComponent = mockk()
    private lateinit var command: AssistanceCommandSet

    @Before
    fun setup() {
        command = spyk(AssistanceCommandSet())
    ...
    @Test
    fun `when look for PSA executor with actionType enroll then return SetAssistancePSAExecutor`() {
        runTest {
            every { command.actionType } returns Constants.Input.ActionType.ASK

            val executor = command.getPsaExecutor()
            Assert.assertEquals(true, executor is SetAssistancePSAExecutor)
        }
    }
