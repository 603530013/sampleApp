...
class GetAssistanceCommandTest {

    private val middlewareComponent: MiddlewareComponent = mockk()
    private lateinit var command: GetAssistanceCommand

    @Before
    fun setup() {
        command = spyk(GetAssistanceCommand())
    ...
    @Test
    fun `when look for PSA executor with actionType details then return GetInfoForAssistancePsaExecutor`() {
        runTest {
            every { command.actionType } returns Constants.PARAM_ACTION_VEHICLE_DETAILS

            val executor = command.getPsaExecutor()
            Assert.assertEquals(true, executor is GetInfoForAssistancePsaExecutor)
        }
    }
