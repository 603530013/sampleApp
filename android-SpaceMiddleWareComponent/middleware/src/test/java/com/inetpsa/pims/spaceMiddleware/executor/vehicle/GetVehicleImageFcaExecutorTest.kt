...
internal class GetVehicleImageFcaExecutorTest : FcaExecutorTestHelper() {
    ...
    private val vehicle = VehicleDetailsFca(
        vin = "testVin",
        preciseImageURL = "https://image.test.com",
    ...
    override fun setup() {
        super.setup()
        executor = spyk(GetVehicleImageFcaExecutor(baseCommand))
    }
    ...
    fun `when execute then make a get API call`() {
        ...
        coEvery { communicationManager.get<VehicleDetailsFca>(any(), any()) } returns NetworkResponse.Success(vehicle)
        val queries = mapOf(
        ...
        runTest {
            ...
            verify {
                executor.request(
                    type = eq(VehicleDetailsFca::class.java),
            ...
            Assert.assertEquals(vehicle.preciseImageURL, success.imageUrl)
            Assert.assertEquals(vehicle.modelDescription, success.model)
        }
    }
