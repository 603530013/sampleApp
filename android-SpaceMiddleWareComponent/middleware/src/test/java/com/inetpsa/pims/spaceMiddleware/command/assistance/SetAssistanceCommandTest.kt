...
@Deprecated("try to switch to use this class AssistanceCommandSetTest")
class SetAssistanceCommandTest {

    private val middlewareComponent: MiddlewareComponent = mockk()
    private lateinit var command: SetAssistanceCommand

    @Before
    fun setup() {
        command = spyk(SetAssistanceCommand())
    ...
    @Test
    fun `when look for PSA executor with actionType enroll then return SendInfoForAssistancePsaExecutor`() {
        runTest {
            every { command.actionType } returns Constants.PARAM_ACTION_VEHICLE_ENROLL

            val executor = command.getPsaExecutor()
            Assert.assertEquals(true, executor is SendInfoForAssistancePsaExecutor)
        }
    }
