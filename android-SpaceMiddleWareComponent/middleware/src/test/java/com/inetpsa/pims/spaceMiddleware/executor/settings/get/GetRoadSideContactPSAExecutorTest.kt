...
internal class GetRoadSideContactPSAExecutorTest : PsaExecutorTestHelper() {

    private lateinit var executor: GetRoadSideContactPSAExecutor
    ...
    private val assistanceRsaPhone = mapOf(
        Constants.Output.Common.PHONES to mapOf(Constants.PRIMARY to "00-88-99-77-11")
    )
    ...
    override fun setup() {
        ...
        executor = spyk(GetRoadSideContactPSAExecutor(baseCommand))
    }
    ...
    fun `when execute then make a network call with success response`() {
        runTest {
            ...
            Assert.assertEquals(assistanceRsaPhone, success.response)
        }
    }
}
