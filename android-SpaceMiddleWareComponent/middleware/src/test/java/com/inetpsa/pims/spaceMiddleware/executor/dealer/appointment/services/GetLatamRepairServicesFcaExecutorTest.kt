...
internal class GetLatamRepairServicesFcaExecutorTest : FcaExecutorTestHelper() {

    private lateinit var executor: GetLatamRepairServicesFcaExecutor
    ...
    fun setUp() {
        ...
        executor = spyk(GetLatamRepairServicesFcaExecutor(middlewareComponent))
    }
    ...
    fun `when execute then make a get API call`() {
        ...
        runTest {
                ...
                executor.request(
                    ...
                    urls = eq(
                        arrayOf(
                            ...
                            "/servicescheduler/repairservices"
                        )
                    ),
                    queries = eq(queries)
                )
            ...
            coVerify(exactly = 1) { BookingOnlineCache.write(ServiceType.Repair, dealerServiceResponse.services) }
